...
class RewardSystemIntegrationTest {
    ...
    void testGetRewardsForExistingCustomer() throws Exception {
        mockMvc.perform(get("/api/rewards/{customerId}", 1)
                ...
                .andExpect(jsonPath("$.customerName").value("Saurabh"))
    ...
    void testGetRewardsForNonExistentCustomer() throws Exception {
    	MvcResult result = mockMvc.perform(get("/api/rewards/{customerId}", 999)
                .param("startDate", "2024-11-01")
                .param("endDate", "2025-01-31")
                .contentType(MediaType.APPLICATION_JSON))
    ...
    void testGetRewardsWithInvalidDateRange() throws Exception {
        mockMvc.perform(get("/api/rewards/{customerId}", 1)
                ...
                .param("endDate", "2024-11-01")  // Invalid: End date before start date
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Invalid date range: startDate (2025-02-01) must be before or equal to endDate (2024-11-01)"));
    }
}
